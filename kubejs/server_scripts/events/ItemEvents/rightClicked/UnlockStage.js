...
ItemEvents.rightClicked(event => {
    ...
    tags.forEach(tag => {
        ...
        if (tag.namespace == "greedycraft") {
            ...
            if (tag.getPath().startsWith("unlock_stage/")) {
                // 截断字符串，留下截断名称
                let stage = tag.getPath().replace("unlock_stage/", "")
                ...
                if (!(AStages.playerHasStage(stage, player))) {
                    ...
                    // 生成粒子
                    level.spawnParticles("minecraft:totem_of_undying", true, player.getX(), player.getY(), player.getZ(), 2.0, 2.0, 2.0, 1000, 5)
