...
EntityEvents.beforeHurt(event => {
    ...
    if (entity.isUndead()) {
        ...
        if (source.getType() == "inFire" || source.getType() == "onFire" || source.getType() == "lava") {
            ...
            if (entity.level.canSeeSky(entity.pos) && entity.level.isDay() && !(global.bossList.includes(entity.type))) {
    ...
    if (source.getType() == "thorns") {
        ...
        if (source.player) {
            ...
            // 如果实体是 Boss 则取消受伤
            if (global.bossList.includes(entity.type)) {
                event.cancel()
            }
        }
    }
    ...
    if (source.actual && global.spiderList.includes(source.actual.type)) {
        // 如果受伤的不是玩家则直接返回
        if (!(entity.isPlayer())) {
            return
        }
    ...
    if (source.getType() == "arrow" || source.getType() == "twilightforest.leafBrain") {
        // 排除 Boss
        if (source.actual && !(global.bossList.includes(source.actual.type))) {
            // 根据游戏难度设置伤害
            event.setDamage(event.damage * (1.0 + 0.003 * entity.getData(PsAttachmentTypes.DIFFICULTY)))
            // 如果造成伤害的是骷髅类实体则伤害翻倍
            if (source.actual && global.skeletonList.includes(source.actual.type)) {
